...
fn cmd_oci_runtime(pid: ProcessId, args: &str) -> Result<Option<String>, ShellError> {
    ...
    match (parts.next(), parts.next(), parts.next(), parts.next()) {
        ...
        (Some("start"), Some(id), None, None) => {
            let pid = oci_runtime::start_container(id).map_err(ShellError::OciRuntime)?;
            // NOTE: This wait+TTY-attach mode is always on today, but should be conditional on
            // `process.terminal = true` from the OCI bundle config in the future.
            // NOTE: Waiting by host PID is a pragmatic interim choice; container PID spaces will
            // eventually diverge from the host, so the wait logic should move to container state.
            oci_runtime::wait_for_exit(pid);
            Ok(None)
        }
...
mod tests {
    ...
    #[kernel_test_case]
    fn oci_runtime_start_waits_for_exit_and_attaches_tty() {
        ...
        handle
            .write(
                br#"{"ociVersion":"1.0.2","root":{"path":"rootfs"},"process":{"cwd":"/","args":["/demo"],"user":{"uid":0,"gid":0},"terminal":true}}"#,
            )
            .expect("write config");

        let shell_pid = PROCESS_TABLE
            .create_kernel_process("shell-oci-test")
            .expect("create process");
        ...
        run_command(shell_pid, "oci-runtime create demo /bundle").expect("create");
        run_command(shell_pid, "oci-runtime start demo").expect("start");
        ...
        assert!(
            output
                ...
                .any(|chunk| chunk == expected),
            "output missing container IO: {:?}",
            output
        );

        let container = CONTAINER_TABLE.get("demo").expect("container");
        let pid = container.state().pid.expect("pid");
        assert_eq!(
            PROCESS_TABLE.thread_count(pid).unwrap_or(0),
            0,
