...
- `tar <archive>` extracts a POSIX tar file into the caller’s current working directory on the writable ramfs mount. Only regular files and directories are supported, paths must be relative (no `..` or absolute prefixes), and checksum validation guards against malformed archives.
