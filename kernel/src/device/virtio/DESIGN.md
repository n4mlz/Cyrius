...
- Device-specific configuration space is exposed via `read_config`, letting drivers fetch capacity/geometry data without duplicating pointer math.
...
- The helper currently assumes an identity-style physical mapping via `mem::manager::phys_mapper`; this implicit dependency is documented in the module-level docs.
...
## Extensibility Notes
- PCI transport support will share the same queue abstractions; only the register access code will differ. The module layout keeps `mmio`, `queue`, and `dma` separate to accommodate additional transports.
