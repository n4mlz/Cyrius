...
- Linux dispatch implements a minimal set of process/syscall plumbing needed by static busybox:
  `read`, `write`, `open`, `openat`, `close`, `writev`, `stat`, `lstat`, `newfstatat`, `brk`, `poll`
  (TTY-only, blocking until input), `lseek` (currently reports `ESPIPE`), `getcwd`, `getdents64`,
  `chdir`, `fork`, `execve`, `wait4`, `arch_prctl`, `ioctl` (routed through `ControlOps`), `fcntl`
  (dup + FD_CLOEXEC), and basic
  ...
  calls, plus basic socket setup (`socket`, `bind`, `listen`, `accept`, `accept4`, `sendto`,
  `setsockopt`) for TCP/IPv4 streams.
