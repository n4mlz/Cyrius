...
use xtask::{
    ...
    prepare_host_mnt_image, prepare_test_block_image, prepare_test_fat_image, run_qemu,
};
...
fn run_kernel(release: bool) -> Result<()> {
    ...
    let mnt_image = prepare_host_mnt_image()?;
    let status = run_qemu(&img, false, &[mnt_image])?;
    if !status.success() {
